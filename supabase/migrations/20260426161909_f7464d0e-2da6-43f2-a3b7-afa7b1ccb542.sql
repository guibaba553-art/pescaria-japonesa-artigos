...
BEGIN
  ...
  -- em_preparo -> enviado | aguardando_envio | retirado | cancelado
  IF v_old_status = 'em_preparo' AND NEW.status IN ('enviado', 'aguardando_envio', 'retirado', 'cancelado') THEN
    RETURN NEW;
  END IF;

  -- aguardando_envio -> enviado | em_preparo (volta) | cancelado
