-- Impede transição manual de aguardando_pagamento → em_preparo.
-- Apenas edge functions (service_role) podem fazer essa transição
-- (via webhooks ou verify-payment). O painel admin não pode pular
-- a verificação de pagamento.
...
BEGIN
  ...
  -- Transição aguardando_pagamento → em_preparo só via service_role
  -- (webhooks, verify-payment). Admin/employee não podem pular a verificação.
...
-- Recria o trigger (DROP + CREATE para garantir que aponte para a função atualizada)
